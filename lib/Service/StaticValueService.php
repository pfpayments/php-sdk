...
class StaticValueService {
	 ...
	 * @return \PostFinanceCheckout\Sdk\Model\StaticValue[]
	 */
	public function all() {
	...
	public function allWithHttpInfo() {
		...
		try {
			...
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\PostFinanceCheckout\Sdk\Model\StaticValue[]', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
				case 200:
					$responseObject = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\PostFinanceCheckout\Sdk\Model\StaticValue[]', $e->getResponseHeaders());
