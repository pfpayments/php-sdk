...
class CountryStateService {
	 ...
	 * @return \PostFinanceCheckout\Sdk\Model\RestCountryState[]
	 */
	public function all() {
	...
	public function allWithHttpInfo() {
		...
		try {
			...
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\PostFinanceCheckout\Sdk\Model\RestCountryState[]', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
				case 200:
					$responseObject = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\PostFinanceCheckout\Sdk\Model\RestCountryState[]', $e->getResponseHeaders());
