...
class TransactionCommentService {
	 ...
	 * @return \PostFinanceCheckout\Sdk\Model\TransactionComment[]
	 */
	public function all($spaceId, $transactionId) {
		return $this->allWithHttpInfo($spaceId, $transactionId)->getData();
	}
	 ...
	 * @return ApiResponse
	 */
	public function allWithHttpInfo($spaceId, $transactionId) {
		...
		if ($transactionId === null) {
			throw new \InvalidArgumentException('Missing the required parameter $transactionId when calling all');
		}
		// header params
		$headerParams = array();
		...
		try {
			...
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\PostFinanceCheckout\Sdk\Model\TransactionComment[]', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
				case 200:
					$responseObject = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\PostFinanceCheckout\Sdk\Model\TransactionComment[]', $e->getResponseHeaders());
